...
while True:
    ...
    print(soup.find_all('table')[4].find_all('tr')[
          1].find_all('td')[1].text.strip())  # name
    for i in range(1, 6):
        s += int(soup.find_all('table')
                 [5].find_all('tr')[i].find_all('td')[4].text)  # sum
        print(int(soup.find_all('table')[5].find_all('tr')[
              i].find_all('td')[4].text), end=' ')  # print marks
